...
class VirtualInputImpl {
     ...
     * moves so the 10px auto-close band is never crossed).
     */
    moveTo(tx: number, ty: number, clamp?: ScreenRect): Promise<void> {
        this.touch();
        ...
        return new Promise(resolve => {
            this.trajectory = { points, idx: 0, clamp: clamp ?? null, resolve, fidget: false };
        });
    }
    ...
    private maybeFidget(now: number): void {
        ...
        if (this.rand.chance(1 / (this.profile.fidgetMeanS * 50))) {
            ...
            const points = windMouse(this.x, this.y, tx, ty, this.profile, this.rand);
            this.trajectory = { points, idx: 0, clamp: null, resolve: () => {}, fidget: true };
            return;
        }
