...
export default class SyntheticInputDriver implements InputDriver {
    ...
    private enqueue(job: () => Promise<boolean>): Promise<boolean> {
        const run = this.queue.then(() => (this.cancelled ? false : job()));
    ...
    private async resolveOp(spec: ResolveSpec): Promise<boolean> {
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            ...
            // at the target's current box with a small displacement (forces a
            // fresh pointermove -> buildMinimenu and shifts off a dead pixel)
            // before burning a whole attempt
            let found = await this.waitForMenuEntry(spec.match, MENU_POLL_MS);
            for (let correction = 0; !found && correction < 4; correction++) {
                if (this.cancelled) {
                    return false;
                }
                ...
                const p = this.jitterInBox(fresh);
                await vi.moveTo(p.x + vi.rand.gaussian() * 2, p.y + vi.rand.gaussian() * 2);
