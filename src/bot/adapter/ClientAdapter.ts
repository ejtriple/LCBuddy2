...
export const reader = {
    ...
    projectFine(fineX: number, fineZ: number, height: number): ScreenPoint | null {
        ...
        const y = raw.getAvH(fineX, fineZ, raw.minusedlevel) - height;
        ...
        const sx = VIEW_ORIGIN_X + (((dx << 9) / dz) | 0);
        const sy = VIEW_ORIGIN_Y + (((dy << 9) / dz) | 0);
        if (sx < 0 || sx > VIEW_W || sy < 0 || sy > VIEW_H) {
            return null;
        }

        return { x: sx + VIEW_X, y: sy + VIEW_Y };
    },
    ...
    /** Clickable box around a loc tile (aimed at the model's lower trunk). */
    locScreenBox(lx: number, lz: number): ScreenRect | null {
