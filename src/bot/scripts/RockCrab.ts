...
export default class RockCrab extends TaskBot {
    ...
    override async onStart(): Promise<void> {
        ...
        this.add(
            new RandomEventTask(msg => this.log(msg), ['rock crab']),
...
class GoToField implements Task {
    ...
    async execute(): Promise<void> {
        ...
        const ok = await Traversal.walkTo(FIELD, { radius: 4, timeoutMs: 240000, log: m => this.bot.log(`  ${m}`) });
        if (ok) {
            this.bot.clearWakes();
        }
    }
...
class DeathRecovery implements Task {
    ...
    async execute(): Promise<void> {
        ...
        this.bot.countDeath();
        this.bot.log('died! waiting for respawn, then web-walking back to the field');
        await Execution.delayUntil(() => Game.ingame() && Game.tile() !== null, 20000);
        await Execution.delayTicks(3);
        await Traversal.walkTo(FIELD, { radius: 4, timeoutMs: 300000, log: m => this.bot.log(`  ${m}`) });
        this.bot.died = false;
    }
}
