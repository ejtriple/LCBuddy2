...
export default class GatheringBot extends TaskBot {
    ...
    override async onStart(): Promise<void> {
        ...
        this.log(`gathering '${this.target}' (${this.action}) within ${this.leash} of ${this.anchor}, dropping *${this.dropMatch}* when full`);
        ...
        this.add(new RandomEventTask(msg => this.log(msg)), new ContinueDialog(this), new DropProduct(this), new Gather(this), new ReturnToAnchor(this));
    }
...
class DropProduct implements Task {
    ...
    private products() {
        ...
        return Inventory.items().filter(i => i.name?.toLowerCase().includes(kw));
    }

    validate(): boolean {
        return Inventory.isFull() && this.products().length > 0;
    }
