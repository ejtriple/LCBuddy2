...
export default class ChaosDruidKiller extends TaskBot {
    ...
    override async onStart(): Promise<void> {
        ...
        this.add(
            new RandomEventTask(msg => this.log(msg)),
...
/** Death -> wait out respawn, then descend back to the druids. */
class DeathRecovery implements Task {
    ...
    async execute(): Promise<void> {
        ...
        this.bot.countDeath();
        this.bot.log('died! waiting for respawn, then heading back down to the druids');
        await Execution.delayUntil(() => Game.ingame() && Game.tile() !== null, 20000);
        await Execution.delayTicks(3);

        // respawn is on the surface; climb back down and walk to the anchor
        if ((Game.tile()?.z ?? 0) < UNDERGROUND_Z) {
            await this.bot.descendToDungeon();
        }
        const here = Game.tile();
        if (here && this.bot.getAnchor().distanceTo(here) > 3 && here.z > UNDERGROUND_Z) {
...
class BankRun implements Task {
    ...
    private async openBank(): Promise<boolean> {
        // a booth only opens reliably when we fire op2 from a tile directly
        // adjacent to it (otherwise the approach pathing wanders the counter),
        // so step exactly onto the stand tile and operate the adjacent booth
        for (let attempt = 0; attempt < 4 && !Bank.isOpen(); attempt++) {
            let booth = Locs.query().name(BANK.name).where(l => l.distance() <= 1).nearest();
            if (!booth) {
                await Traversal.walkTo(BANK.stand, { radius: 0, timeoutMs: 30000, log: m => this.bot.log(`  ${m}`) });
                await Execution.delayTicks(1);
                booth = Locs.query().name(BANK.name).where(l => l.distance() <= 1).nearest();
                if (!booth) {
                    continue;
                }
            }
            const op = booth.actions().find(a => a.toLowerCase() === BANK.op.toLowerCase()) ?? booth.actions().find(a => /^use/i.test(a)) ?? booth.actions()[0];
            if (op) {
