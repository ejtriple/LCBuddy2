...
// balance, run GnomeCourse, and assert agility xp is gained (an obstacle was
// traversed via the OPLOC path).
...
try {
    ...
    while (Date.now() < deadline && !gained) {
        ...
        gained = (await agiXp()) > baseXp;
    }
    await page.getByRole('button', { name: 'Stop' }).click().catch(() => {});
    if (!gained) fail('no agility xp — obstacle traversal did not work');
    console.log('\nresult: GnomeCourse traversed an obstacle (agility xp gained) — PASS');
